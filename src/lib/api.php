...
class API{
	...
	public function init(){
		...
		if(!is_dir(dirname(__FILE__,3).'/git')){
			mkdir(dirname(__FILE__,3).'/git');
		}
		if(!is_dir(dirname(__FILE__,3).'/users')){
			mkdir(dirname(__FILE__,3).'/users');
		}
		if(!is_dir(dirname(__FILE__,3).'/repos')){
			mkdir(dirname(__FILE__,3).'/repos');
		}
		...
		if(!file_exists(dirname(__FILE__,3).'/git/.htaccess')){
			$htaccess=fopen(dirname(__FILE__,3).'/git/.htaccess', 'w');
			fwrite($htaccess, "Order deny,allow\n");
			fwrite($htaccess, "Deny from all\n");
			fclose($htaccess);
		}
		...
		if(!file_exists(dirname(__FILE__,3).'/repos/.htaccess')){
			$htaccess=fopen(dirname(__FILE__,3).'/repos/.htaccess', 'w');
			fwrite($htaccess, "Order deny,allow\n");
			fwrite($htaccess, "Deny from all\n");
			fclose($htaccess);
		}
		...
		if(isset($_SESSION['lsp'])){
			$this->User = json_decode(file_get_contents(dirname(__FILE__,3).'/users/'.$_SESSION['lsp'].'.json'),true);
			$this->Login = TRUE;
		}
	}
